...
class CIRCODataset(Dataset):
    ...
    def __init__(self, split: Literal['val', 'test'], mode: Literal['relative', 'classic'],
                 preprocess: callable, config):
        """
        Args:
        ...
        with open(os.path.join(data_path , 'COCO2017_unlabeled' , "annotations" , "image_info_unlabeled2017.json"), "r") as f:
            imgs_info = json.load(f)

        self.img_paths = [os.path.join(data_path , 'COCO2017_unlabeled' , "unlabeled2017" , img_info["file_name"]) for img_info in
                          imgs_info["images"]]
